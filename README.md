...
| `COPY_POLL_MS` | `2500` | Период опроса истории кошельков копитрейда (мс, пол 1000). Фолбэк-режим |
| `POLYGON_WSS` | — | WSS-RPC Polygon (Alchemy/Infura/QuickNode). Включает ⚡ ТУРБО копитрейда (ончейн-push, без задержки Data API). Без него работает обычный опрос |
| `COPY_REAL_LIVE` | `false` | Мастер-флаг реального исполнения копитрейда. ВЫКЛ = реальный режим считается «бумажным» (paper). Включать только осознанно |

### Копитрейдинг Polymarket
До 15 кошельков. Каждый — отдельная карточка с demo/real счётом, итоговым P&L на дистанции,
WR, числом сделок, открытыми позициями и лентой активности. Карточки сортируются по P&L
(самый профитный сверху, бейдж «★ лучший»). Копируются только сделки ПОСЛЕ добавления кошелька.
- **ТУРБО (⚡)**: при заданном `POLYGON_WSS` бот слушает событие `OrderFilled` контрактов биржи
  Polymarket на Polygon — сделка прилетает в момент включения в блок (push, без rate-limit).
  При отвале WS автоматически реконнектится, а опрос Data API подстраховывает (фолбэк).
- **REAL = PAPER** по умолчанию. Реальные ордера ставятся только при `COPY_REAL_LIVE=true`
  и настроенном кошельке/ключах, с тем же полом баланса, что и BTC-реал.
- **Размер ставки — единый калькулятор** (применяется ко всем кошелькам): «Фиксированная»
  (myMax$ на вход) или «Плавающая» (myMax × X/MaxT, X — шэры их сделки, MaxT — их макс.;
  но не менее minShares шэров).
- **REAL-баланс копитрейда = реальный USDC кошелька** (общий с BTC-реалом), а не paper-$1000.
  Пока `COPY_REAL_LIVE=false` копи-PnL по REAL — бумажный (предпросмотр).
- **Сброс всех демо-результатов** (кнопка «↺ сброс демо» в шапке стратегий): балансы и
  счётчики обнуляются, но **журнал сделок сохраняется** (переживает сброс и рестарт).
